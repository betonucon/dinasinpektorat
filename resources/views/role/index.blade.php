			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<form action="{{url('master-data/role/store')}}" id="data-role" method="post" enctype="multipart/form-data">
							@csrf
...
@push('ajax')       
        ...
        var handleDataTableFixedHeader = function() {
            ...
            if ($('#data-table-fixed-header').length !== 0) {
                var table=$('#data-table-fixed-header').DataTable({
                    ...
                    responsive: true,
                    ajax:"{{ url('master-data/role/get-data')}}",
					columns: [
	...
	<script>
		function tambah(id){
			...
			$.ajax({
				type: 'GET',
				url: "{{url('master-data/role/modal')}}",
				data: "id="+id,
		...
		function hapus(id){
			...
			if (result.isConfirmed) {
				$.ajax({
					type: 'GET',
					url: "{{url('master-data/role/delete-data')}}",
					data: "id="+id,
					success: function(msg){
						Swal.fire(
							'Successful!',
		...
		function simpan_data(){
			...
			var form=document.getElementById('data-role');
				$.ajax({
					type: 'POST',
					url: "{{url('master-data/role/store')}}",
